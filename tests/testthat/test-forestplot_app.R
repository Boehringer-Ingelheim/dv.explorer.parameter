...
app_args <- dv.explorer.parameter::mock_app_forest(dry_run = TRUE)
...
local({
  ...
  test_that(
    "forestplot table is included according to selection. Continuous" |>
      vdoc[["add_spec"]](c(
        specs$forest_module$forest_plot,
        specs$forest_module$table,
        specs$forest_module$composition
      )),
    {
      l_inputs <- list()
      l_inputs[[ID$INPUT$FOREST_KIND]] <- "Spearman Correlation"
      app$set_inputs(!!!l_inputs)
      app$wait_for_idle()

      exported <- app$get_value(export = "not_ebas-output_arguments")

      exported_table <- shiny::isolate(exported[[FP_ID$TABLE_LISTING]][["arguments"]]())
      expect_snapshot(exported_table["ds"], cran = TRUE)
      table_html <- app$get_html(paste0("#", ID$OUTPUT$TABLE))
      expect_true(grepl("^<div class=\"datatables", table_html))

      exported_svg <- shiny::isolate(exported[[FP_ID$FOREST_SVG]][["arguments"]]())
      expect_snapshot(exported_svg, cran = TRUE)
  ...
  test_that(
    "forestplot table is included according to selection. Odds" |>
      vdoc[["add_spec"]](c(
        specs$forest_module$forest_plot,
        specs$forest_module$table,
        specs$forest_module$composition
    ...
    {
      l_inputs <- list()
      l_inputs[[ID$INPUT$FOREST_KIND]] <- "Odds Ratio"
      app$set_inputs(!!!l_inputs)
      app$wait_for_idle()
