...
test_that("wfphm_server prints message for 0 rows group_dataset", {
  ...
  shiny::testServer(
    ...
    ),
    {
      ...
      err <- expect_error(isolate(v_group_dataset()), regexp = "group_dataset: Dataset has 0 rows")
    }
  )
})

test_that("wfphm_server prints message for 0 rows bm_dataset", {
  data <- test_data(anlfl_flags = FALSE)
  ...
  shiny::testServer(
    ...
    {
      # check that the error message produced is as expected
      err <- expect_error(isolate(v_bm_dataset()), regexp = "bm_dataset: Dataset has 0 rows")
    }
  )
})
