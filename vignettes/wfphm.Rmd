...
[dv.wfphm::mod_wfphm()] module uses several arguments with the following two being mandatory and
...
Refer to
`dv.wfphm::mod_wfphm()` for the complete list of arguments and their description.

# Input dataset

The `wfphm` module is designed to work with [ADaM Basic Data Structure
(BDS) using
...
or "PCHG". See `dv.wfphm::wfphm_server` for a full list)
...
[Plotting Parameter](../articles/dv-wfphm.html#plotting-parameters)
section in the Waterfall plot with the difference that the *Category*
and *Parameter* menus now accept selecting multiple values. The
*Transformation* menu will be covered in a separate section below.
...
`dv.wfphm::wfphm_server` for a full list) but the user can include
...
example_dataset <- list(
  "adbm" = tibble::tibble(
    ...
    safety_category = factor(c("PC1", "PC2", "PC1", "PC2", "PC1", "PC2")),
    safety_parameter = factor(c("P1", "P2", "P1", "P2", "P1", "P2")),
    timepoint = factor(c("V1", "V2", "V1", "V2", "V2", "V1")),
    log_aval = log(c(11, 12, 21, 22, 31, 3))
  ),
  "adsl" = tibble::tibble(
    SUBJID = factor(c(1, 2, 3)),
    WEIGHT = c(10, 20, 30),
...
module_list <- list(
  "WFPHM" = dv.wfphm::mod_wfphm(
    ...
    bm_dataset_name = "adbm",
    group_dataset_name = "adsl",
    cat_var = "safety_category",
    par_var = "safety_parameter",
    subjid_var = "subject",
    value_vars = c("log_aval"),
    visit_var = "timepoint"
  )
)

dv.manager::run_app(
  data = list(DS = example_dataset_non_standard),
  module_list = module_list,
  filter_data = "adsl",
  filter_key = "subject" # We changed this one too, see dv.manager filtering doc for this change
)
```
