...
-   A set of boxplot/violin charts with different custom groupings and optionally showing the individual data points.
    The violin plot option can be restricted by the App Creator (see `allow_violin` below).
...
**Optional Arguments controlling visible features**

-   `allow_violin`: If `FALSE` (default: `TRUE`), the violin plot option is removed from the "Other" menu and
    violin plots cannot be displayed.

-   `allow_pvalue`: If `FALSE` (default: `TRUE`), the "Data Significance" tab, which contains p-values comparing
...
A table containing a listing of the population of a single x-axis value selected by clicking, and a table containing a
...
Since this table contains p-values, it is only displayed if the App Creator sets `allow_pvalue = TRUE`
(the default) when calling `mod_boxplot()`.
