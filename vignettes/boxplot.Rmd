...
dv.manager::run_app(
  ...
  module_list = list(
    Boxplot = dv.explorer.parameter::mod_boxplot(
      ...
      default_x_axis_vals = levels(adbm_dataset[["AVISIT"]])[2:4],
      #default_x_axis_vals = unique(as.character(adbm_dataset$AVISIT))[2:4],
...
adlb_dataset <- pharmaverseadam::adlb |>
  dplyr::filter(is.na(DTYPE))
adsl_dataset <- pharmaverseadam::adsl

dv.manager::run_app(
  data = list(dummy = list(adlb = adlb_dataset, adsl = adsl_dataset)),
  module_list = list(
    Boxplot = dv.explorer.parameter::mod_boxplot(
      "boxplot",
      bm_dataset_name = "adlb",
      group_dataset_name = "adsl",
      cat_var = "PARCAT1",
      par_var = "PARAM",
      value_vars = c("AVAL", "CHG"),
      x_axis_vars = "VISIT",
      subjid_var = "USUBJID",
      default_cat = "CHEMISTRY",
      default_par = c("Alanine Aminotransferase (U/L)", "Albumin (g/L)"),
      #default_x_axis_vals = unique(adlb_dataset[["VISIT"]])[2:4],
