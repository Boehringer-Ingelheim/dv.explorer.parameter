...
- Allows selecting several columns from all numerical columns, from now
  on **numerical_selection**.
...
- Pivot the dataset to a longer format such that each row have:
  ...
  - column `z`: the value of the **numerical_selection** in `y` for the
    value in `x`
...
- `palette` is hardcoded to
  `RColorBrewer::brewer.pal(n = 8, name = "Accent")` after 8 categories
  colors are repeated
