...
let chart = function (Shiny, 
//@ts-ignore
d3, r2d3, data, svg, t_width, t_height, options, _theme, _console) {
    [];
    // CONSTANTS ===========================================================
    ...
    // OnRender Callback =====================================================
    const onRenderCallback = function (data, _svg, t_width, t_height, options) {
        ...
        const y_baseline = options.y_baseline !== undefined ? options.y_baseline : 0;
        ...
        mySvg.attr("transform", "translate(" + options.margin.left + "," + options.margin.top + ")");
        // X ====
        ...
        if (options.x_axis !== null) {
            ...
            if (x_title !== null) {
                x_axis_el
                    ...
                    .attr("transform", `translate(${dvd3h.mid_point(x_scale.range()[0], x_scale.range()[1])}, ${dvd3h.calc_el_height(x_axis_el) + C.AXIS.X.TITLE.VPADDING})`)
                    .text(x_title);
            }
        ...
        if (options.y_axis !== null) {
            ...
            if (y_title !== null) {
                y_axis_el
                    ...
                    // Translate to the width of the element plus padding
                    .attr("transform", `translate(${-dvd3h.calc_el_width(y_axis_el) - C.AXIS.Y.TITLE.HPADDING}, ${dvd3h.mid_point(y_scale.range()[0], y_scale.range()[1])})rotate(${C.AXIS.Y.TITLE.ROTATION})`)
                    .text(y_title);
            }
        ...
        if (options.z_axis !== null) {
            ...
            z_axis_el
                ...
                .append(() => dvd3h.get_legend(d3, z_scale, {
                height: height,
                marginTop: 5,
                marginBottom: 5,
                marginLeft: 5,
        ...
        if (prev_tooltip !== null)
            prev_tooltip.remove();
        // create a tooltip
        let tooltip = dvd3h.def_tooltip.append(d3.select("#" + ns(options.parent)), ns(C.TOOLT.ID));
        const mouseover = dvd3h.def_tooltip.mouseover_factory(tooltip);
        const mousemove = dvd3h.def_tooltip.mousemove_factory(tooltip, eval(options.msg_func));
        ...
        myLabels
            ...
            .attr("font-size", Math.min(C.BAR.LABEL.MAX_SIZE, x_scale.bandwidth() * C.BAR.LABEL.PADDING_PERC))
